...
final class NodeLabelTestDriver {
  ...
  final class StartHandler implements EventHandler<StartTime> {
    ...
    public void onNext(final StartTime startTime) {
      final EvaluatorRequest reqToMylabel = EvaluatorRequest.newBuilder()
          ...
          .setMemory(64)
          .setNumberOfCores(1)
          .setNodeLabelExpression(NodeLabelTestDriver.this.nodeLabelExpression)
          .build();
      LOG.log(Level.INFO, "Requested Evaluator with node label: {0}", reqToMylabel.getNodeLabelExpression());
  ...
  class StopHandler implements EventHandler<StopTime> {
    ...
    public void onNext(final StopTime stopTime) {
      ...
      if (NodeLabelTestDriver.this.labeledContainerCount != 1) {
        throw new RuntimeException("Asked for one labeled container, but got " +
            NodeLabelTestDriver.this.labeledContainerCount + "default container.");
      }
    }
  }
}
